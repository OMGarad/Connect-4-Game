...
class Q_Learning:
    def __init__(self, player, win_reward=5, draw_reward=1, eps=0.5, alpha=0.1, gamma=1):
        self.table = np.zeros((3**(r*c), c))
        self.player = player
        self.win_reward = win_reward
        self.draw_reward = draw_reward
        self.eps = eps
        self.alpha = alpha
        self.gamma = gamma
        self.r = np.ones((3**(r*c))) * (-1)
        for i in range(3**(r*c)):
            if get_winner(i) == self.player:
                self.r[i] = self.win_reward
            elif get_winner(i) > 0:
                self.r[i] = -self.win_reward
            elif len(get_actions(get_state(i))):
                self.r[i] = self.draw_reward

    def update_values(self, hash, action):
        hash_next = get_hash(apply_action(
            get_state(hash), action, self.player))
        self.table[hash][action] += self.alpha * \
            (self.r[hash] + self.gamma *
    ...
    def get_best_play(self, state):
        ...
        self.update_values(hash, action)
        return action
...
if __name__ == "__main__":
    ...
    for _ in range(10):
        mc0 = Player(player=1, simulations=0)
        mc200 = Player(player=2, simulations=200)
        game(mc0, mc200)
    mc0.update_player(2)
